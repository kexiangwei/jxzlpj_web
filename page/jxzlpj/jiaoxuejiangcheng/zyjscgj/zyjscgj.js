            ...
            if(data.isAuthSubmit > 0){ //拥有提交权限
                ...
                var myself_table = table.render({
                    ...
                    ,cols : [[ //表头
                        ...
                        ,{field: 'majorCode', title: '专业代码', width:150, sort:true}
                        ,{field: 'majorType', title: '专业类型', width:150, sort:true}
                        ,{field: 'majorLeader', title: '专业负责人', width:150, sort:true}
                        ,{field: 'leaderUnit', title: '所在单位', width:150, sort:true}
                        ,{field: 'passTime', title: '获批/通过时间', width:150, sort:true}
                ...
                let active = {
                    search: function(){
                        myself_table.reload({
                            where: {
                                'majorName': $(".myself_search input[name='majorName']").val()
                            ...
                            layer.open({
                                ...
                                ,success: function(layero, index){
                                    ...
                                    form.on('submit(toSubmitEidtForm)', function(data){
                                        $.post(requestUrl+'/jxjc_zyjscgj/insert.do', data.field, function(result_data){
                ...
                table.on('tool(myself_table)', function(obj){
                    ...
                    if (obj.event === 'detail_dataInfo') {
                        ...
                        detail_shenheProcess('教学奖惩-专业建设成果奖-查看审核流程',data);
                        ...
                        layer.open({
                            ...
                            ,success: function(layero, index){
                                ...
                                //监听表单提交
                                form.on('submit(toSubmitEidtForm)', function(data){
                                    $.post(requestUrl+'/jxjc_zyjscgj/update.do', data.field, function(result_data){
                                        layer.msg(result_data.msg, { offset: '100px'}, function () {
                                            /*if(result_data.code == 200){
                                                myself_table.reload();//重新加载表格数据
                                            }*/
                        ...
                        layer.confirm('删除后不可恢复，真的要删除么？', {icon: 3, title:'提示', offset: '100px'}, function(index) {
                            $.post(requestUrl+'/jxjc_zyjscgj/delete.do', { "code": data.code},function(result_data){
                                layer.msg(result_data.msg, { offset: '100px'}, function () {
                                    if(result_data.code == 200){
                                        myself_table.reload();//重新加载表格数据
                                    }
            ...
            if(data.isAuthShenhe > 0){ //拥有审核权限
                ...
                let active = {
                    search: function(){
                        other_table.reload({
                            where: {
                                'majorName': $(".other_search input[name='majorName']").val()
                ...
                table.on('tool(other_table)', function(obj){
                    ...
                    if (obj.event === 'detail_dataInfo') {
                        ...
                        detail_shenheProcess('教学奖惩-专业建设成果奖-查看审核流程',row_data);
                    }
                });
                ...
                let options = {
                    ...
                    ,success: function(layero, index){
                        ...
                        let html = '<table class="layui-table">\n' +
                            '           <tbody>\n' +
                            '              <tr>' +
                            '                 <td style="width: 80px; text-align: right">专业名称：</td><td style="width: 120px;" colspan="3">'+data.majorName+'</td>' +
                            '              </tr>\n' +

                            '              <tr>' +
                            '                <td style="width: 80px; text-align: right">专业代码：</td><td style="width: 120px;">'+data.majorCode+'</td>' +
                            '                <td style="width: 80px; text-align: right">专业类型：</td><td style="width: 120px;">'+data.majorType+'</td>' +

                            '              </tr>\n' +
                            '              <tr>' +
                            '                <td style="width: 80px; text-align: right">专业负责人：</td><td style="width: 120px;">'+data.majorLeader+'</td>' +
                            '                <td style="width: 80px; text-align: right">所在单位：</td><td style="width: 120px;">'+data.leaderUnit+'</td>' +
                            '              </tr>\n' +
                            '              <tr>' +
                            '                 <td style="width: 80px; text-align: right">获批/通过时间：</td><td style="width: 120px;" colspan="3">'+data.passTime+'</td>' +
                            '              </tr>\n' +
                            '              <tr>' +
                    ...
                    ,success: function(layero, index){
                        ...
                        form.on('submit(toSubmitShenHeForm)', function(formData){
                            $.post(requestUrl+'/toShenhe.do',{
                                'viewName':'v_jxjc_zyjscgj_shenhe'
                                ,'jsonString':JSON.stringify(row_datas)
