...
layui.use(['layer','upload'], function(){
    ...
    var upfileList = $('#upfileList')
        ,upfileList2 = $('#upfileList2');
        ...
        layer.open({
            ...
            ,success: function(layero, index){
                ...
                let upfileIns2 = upload.render({
                        ...
                        obj.preview(function(index, file, result){
                            ...
                            tr.find('.upfile_delete').on('click', function(){
                                $.post(requestUrl+"/deleteFileInfo.do" , {
                                    ...
                                    delete files[index]; //删除对应的文件
                                    tr.remove();
                                    upfileIns2.config.elem.next()[0].value = ''; //清空 input file 值，以免删除后出现同名文件不可选
                                }, "json");
                            });
                        });
                    }
                    ,done: function(res, index, upload){
                        if(res.code == 200){ //上传成功
                            let tr = upfileList2.find('tr#upfile_'+ index)
                                ,tds = tr.children();
                            tr.attr("data-id",res.data.code);//
                            tds.eq(1).html('<span style="color: #5FB878;">已上传</span>');
                            // tds.eq(2).html(''); //清空操作
