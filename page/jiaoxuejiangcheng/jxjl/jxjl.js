        ...
        ,success:function(data) {
            ...
            if(data.isSubmit > 0){ //拥有提交权限
                ...
                var myself_table = table.render({
                    ...
                    ,elem : '#myself_table'
                    ,height : 440
                    ,url: requestUrl+'/jxjl/getPageList.do'
                    ,where:{
                        "userId":function () {
                            return  $.cookie('userId');
                        }
                    ...
                    ,cols : [[ //表头
                        ...
                        ,{field: 'objName', title: '奖项名称', width:120}
                        ,{field: 'level1', title: '奖项级别', width:120}
                        ,{field: 'level2', title: '获得奖项', width:120}
                        ,{field: 'grantUnit', title: '证书授予机构', width:120}
                        ,{field: 'awardDate', title: '获奖日期', width:120}
                    ...
                    ,done: function(res, curr, count){ //数据渲染完的回调
                                    ...
                                    let layerIndex = layer.open({
                                        ...
                                        ,success: function(layero, index){
                                            ...
                                            form.on('submit(toSubmitEidtForm)', function(data){
                                                $.post(requestUrl+'/jxjl/insert.do',data.field,function(result_data){
                        ...
                        table.on('tool(myself_table)', function(obj){
                            ...
                            if (obj.event === 'detail_dataInfo') {
                                ...
                                detail_shenheProcess('教学奖惩-教学奖励-查看审核流程',data);
                                ...
                                let layerIndex = layer.open({
                                    ...
                                    ,success: function(layero, index){
                                        ...
                                        //监听表单提交
                                        form.on('submit(toSubmitEidtForm)', function(data){
                                            $.post(requestUrl+'/jxjl/update.do',data.field,function(result_data){
                                                if(result_data.code == 200){
                                                    myself_table.reload();//重新加载表格数据
                                                }
                                                layer.msg(result_data.msg, { offset: '100px'}, function () {
                                ...
                                layer.confirm('删除后不可恢复，真的要删除么？', {icon: 3, title:'提示', offset: '100px'}, function(index) {
                                    $.post(requestUrl+'/jxjl/delete.do', { "objCode": data.code},function(result_data){
                                        if(result_data.code == 200){
                                            myself_table.reload();//重新加载表格数据
                                        }
                                        layer.msg(result_data.msg, { offset: '100px'},function () {
            ...
            if(data.isShenhe > 0){ //拥有审核权限
                ...
                var other_table = table.render({//数据表格
                    ...
                    ,cols : [[ //表头
                        {type:'checkbox', fixed: 'left'}
                        ,{type:'numbers', title:'序号', width:80, fixed: 'left'}
                        ,{field: 'userId', title: '工号', width:120}
                    ...
                    ,done: function(res, curr, count){
                        ...
                        table.on('tool(other_table)', function(obj){
                            ...
                            if (obj.event === 'detail_dataInfo') {
                                ...
                                detail_shenheProcess('教学奖惩-教学奖励-查看审核流程',row_data);
                            }
                        });
                    }
                });//table end.
                ...
                let options = {
                    ...
                    ,success: function(layero, index){
                        ...
                        let html = '<table class="layui-table">\n' +
                            ...
                            '              </tr>\n' +
                            '              <tr>' +
                            '                <td style="width: 80px; text-align: right">奖项级别：</td><td style="width: 120px;">'+data.level1+'</td>' +
            ...
            let toSubmit = function (row_dataArr){
                layer.confirm('信息提交后不可进行编辑、删除操作，是否继续提交？', {icon: 3, title:'提示', offset: '100px'}, function(index) {
                    $.post(requestUrl+'/jxjl/toSubimt.do',{
                ...
                let index = layer.open({
                    ...
                    ,success: function(layero, index){
                        ...
                        form.on('submit(toSubmitShenHeForm)', function(formData){
                            $.post(requestUrl+'/jxjl/toShenhe.do'
                                ,{
                                    "jsonStr":JSON.stringify(row_dataArr)
